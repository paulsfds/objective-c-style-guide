...
In this example, I will define coding standards for an iOS project (which uses Objective-C). We will take advantage of the fact that code formatting can be automated in XCode by using a plugin. The coding style that I chose is based on the [Chromium](http://www.chromium.org/developers/coding-style) style guide, but I made a few modifications, which can be found in the [`.clang-format`](https://github.com/paulsfds/objective-c-style-guide/blob/master/.clang-format) file. Feel free to make changes as you see fit; All of the options are defined here [LLVM Coding Standards](http://clang.llvm.org/docs/ClangFormatStyleOptions.html).
